...
public class SignInformationActivity extends Activity implements OnClickListener{
	...
	//	顶层的信息按钮;
	private TextView vBack,vHistory,vD1,tvTopic,mState02;
	private Button 	 btnSearch, btnCode, btnOk, btnSign, btnPhoto;
	...
	bid,gid,state,simg,
	folderPath,
	filePath,	tmpPath,sSize,wid,operkind;
	...
	private SQLiteDatabase  	 mDataBase;	  	  // 数据库件;
	...
	private void initView(){
		...
		vHistory	=(TextView) findViewById(R.id.btnFunction);
	...
	private void initEvent(){
		...
		mDataBase	  	=	mSqLiteHelper.getmDB();
	...
	public void onClick(View view) {
		...
		case R.id.sign:
			...
			int width=(int) (mConfigHelper.getScreenWidth()*0.8);
			int heigth=(int) (mConfigHelper.getScreenHeigth()*0.8);
		...
		case R.id.btnSearch:
			if(mThread==null){
				...
				mThread=new MyThread(mGetOrPostHelper,1);
				mThread.start();
			}
			break;
					...
					mBuilder.setPositiveButton(R.string.action_ok, new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface arg0, int arg1) {
							...
							if(mThread==null){
								...
								final CharSequence strDialogBody = getString(R.string.tip_dialog_done);
								mDialog = ProgressDialog.show(mContext, strDialogTitle,strDialogBody, true);
								mThread=new MyThread(mGetOrPostHelper,2);
								mThread.start();
							}						
						}
					});
	...
	public class MyThread extends Thread{
		...
		public MyThread(MTGetOrPostHelper mtGetOrPostHelper,int oper) {
					...
					try {
						...
						do {
							try {		
								...
								String sealno=	obj.getString("seaino");
	...
	private void doPhoto(Context context,Intent intent, String tmppath,String filepath){
		...
		startActivityForResult(intent, MTConfigHelper.NTRACK_SIGN_PHOTO_TO);
	}
