...
public class GetgoodsInformationActivity extends Activity implements OnClickListener {
	...
	private EditText  etSearch 	   // 搜索的框;
					  ;
		...
		if (bid != null && gid != null) {
			...
			vBuilder.setItems(kinds, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface arg0, int position) {
					...
					mBundle.putString("folder", folderPath);
	...
	private void initEvent() {
		...
		operkind		 =	TAG_AUTO;
		vTopic.setText("提货——操作模式	( " + operkind + " )");
		chooseOperKind(mContext, vTopic);
		processOperKind();
		// 系统的配置工具类的添加;
	...
	public void getPhoto_Ggoods() {
		...
		if (mConfigHelper.getfState().equals(Environment.MEDIA_MOUNTED)) {
			if(operkind.endsWith(TAG_AUTO)){
				if(bid!=null&&gid!=null){
					folderPath	= mConfigHelper.getfParentPath()+bid+File.separator+"ggoods"+File.separator+gid;
					gsimg  		= bid+"getgoods"+gid+"file"+java.lang.System.currentTimeMillis();
