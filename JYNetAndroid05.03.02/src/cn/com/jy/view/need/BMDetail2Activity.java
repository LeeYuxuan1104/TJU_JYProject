...
public class BMDetail2Activity extends Activity implements OnClickListener{
    //	全程序内容;
    ...
    private void initEvent(){
        mContext		=	BMDetail2Activity.this;
    ...
    private void doLoadData(int size){
        ...
        while (mCursor.moveToNext()) {
            ...
            String echinaporttime=mCursor.getString(mCursor.getColumnIndex("echinaporttime")).toString(); //
            String eportstorageroomtime=mCursor.getString(mCursor.getColumnIndex("eportstorageroomtime")).toString(); //
            String etimechangeofport=mCursor.getString(mCursor.getColumnIndex("etimechangeofport")).toString(); //
            String echangenumber=mCursor.getString(mCursor.getColumnIndex("echangenumber")).toString(); //
            String efeeofflinetime=mCursor.getString(mCursor.getColumnIndex("efeeofflinetime")).toString(); //
            String erailwayofflinetime=mCursor.getString(mCursor.getColumnIndex("erailwayofflinetime")).toString(); //
            String eactualreturntime=mCursor.getString(mCursor.getColumnIndex("eactualreturntime")).toString(); //
            String cargostatusbox=mCursor.getString(mCursor.getColumnIndex("cargostatusbox")).toString(); //
            // + // 图片
            bid=mCursor.getString(mCursor.getColumnIndex("busiinvcode")).toString();

            listInfo.add("二维码值\t"+gid);
            listInfo.add("提箱地\t"+ecarryaddress);
            listInfo.add("提箱时间\t"+ecarrydate);
            listInfo.add("返到中方口岸时间\t"+echinaporttime);
            listInfo.add("返到口岸库房时间\t"+eportstorageroomtime);
            listInfo.add("口岸换装时间\t"+etimechangeofport);
            listInfo.add("换装车号\t"+echangenumber);
            listInfo.add("下线结费时间\t"+efeeofflinetime);
            listInfo.add("铁路下线时间\t"+erailwayofflinetime);
            listInfo.add("实际回空时间 \t"+erailwayofflinetime);
            listInfo.add("货物状态\t"+cargostatusbox);
